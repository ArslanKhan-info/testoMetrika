...
const Sidebar = ({showSidebar}) => {
  ...
  return (
    <div className={ showSidebar ? `${style.sidebar} ${style.showSidebar}` : style.sidebar}>
          ...
          <ul className={style.navlist}>
            {api.navlist.map((elm,i)=>
                    <li className={style.navlist_item} key={i}>
                      ...
                      {i == 0 && 
                      <>
                        <span className={style.collaps_btn} onClick={()=>setshowCollapsMenu(!showCollapsMenu)}>
                            <span className={style.collaps_icon1}></span>
                            <span className={!showCollapsMenu ? style.collaps_icon2 : `${style.collaps_icon2} ${style.show}`} ></span>
                        </span>
                        {showCollapsMenu &&<CollapsMenu menuList = {api.navlist_inner}/>}
                      </>
                      }
                    </li>
